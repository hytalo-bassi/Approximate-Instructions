 ...
 *
 *       float points[3] = {1.0f, 2.0f, 3.0f};
 *       common_api_metadata_set_array("points", points, 3);
 *       common_api_metadata_set_array_from_history("trace");
 *
 ...
 *   - metadata entries can be numbers, strings, bools, or arrays of
 *     (index, value) number pairs ("dict[str, Any]").
...
/* Sets metadata[key] = [[0, values[0]], [1, values[1]], ...] -- an array
 * of (index, value) pairs, copied from `values` (len elements). Indices
 * are implicit (0..len-1); this matches Python-side
 * `[(i, v) for i, v in enumerate(some_array)]` metadata. */
void common_api_metadata_set_array(const char *key, const float *values, size_t len);

/* Sets metadata[key] = [[0, history[0]], [1, history[1]], ...] -- an
 * array of (index, value) pairs taken directly from the *existing*
 * history list (whatever has been pushed via common_api_history_push_*
 * so far). Non-numeric history entries (strings/bools) are skipped.
 * This matches Python-side `[(i, v) for i, v in enumerate(history)]`
 * metadata, without requiring a second copy of the history data. */
void common_api_metadata_set_array_from_history(const char *key);
...
typedef enum {
    ...
    CAPI_STRING,
    CAPI_BOOL,
    CAPI_ARRAY   /* array of (index, value) number pairs, metadata-only */
} capi_value_type;
...
typedef struct {
    ...
    union {
        ...
        int    boolean;
        struct {
            double *values;   /* heap-owned copy, len elements */
            size_t  len;
        } array;
    } as;
} capi_value;
...
static void capi_free_value(capi_value *v) {
    if (v->type == CAPI_STRING) {
        free(v->as.string);
        v->as.string = NULL;
    } else if (v->type == CAPI_ARRAY) {
        free(v->as.array.values);
        v->as.array.values = NULL;
        v->as.array.len = 0;
    }
}
    ...
    switch (v->type) {
        ...
        case CAPI_ARRAY:
            ...
            for (size_t i = 0; i < v->as.array.len; i++) {
                if (i > 0) fputs(", ", fp);
                fprintf(fp, "[%zu, %.17g]", i, v->as.array.values[i]);
            }
            fputc(']', fp);
            break;
    }
...
void common_api_metadata_set_array(const char *key, const float *values, size_t len) {
    capi_meta_entry *entry = capi_meta_upsert(key);
    entry->value.type = CAPI_ARRAY;
    entry->value.as.array.len = 0;
    entry->value.as.array.values = NULL;

    if (len == 0) return;

    double *copy = (double *)malloc(len * sizeof(double));
    if (!copy) return; /* leaves the metadata entry as an empty array */

    for (size_t i = 0; i < len; i++) {
        copy[i] = (double)values[i];
    }

    entry->value.as.array.values = copy;
    entry->value.as.array.len = len;
}

void common_api_metadata_set_array_from_history(const char *key) {
    capi_history_list *hist = &g_capi_report.history;

    /* Count numeric entries first (strings/bools are skipped -- the
     * (index, value) pairing only makes sense for numbers). Indices in
     * the output correspond to position among the *numeric* entries
     * only, matching Python's `enumerate(history)` when history is
     * built exclusively from history.append(<number>) calls, which is
     * the only shape the transpiler ever generates from history.append(). */
    size_t numeric_count = 0;
    for (size_t i = 0; i < hist->count; i++) {
        if (hist->items[i].type == CAPI_NUMBER) numeric_count++;
    }
    ...
    if (numeric_count == 0) return;

    double *copy = (double *)malloc(numeric_count * sizeof(double));
    if (!copy) return;

    size_t out_i = 0;
    for (size_t i = 0; i < hist->count; i++) {
        if (hist->items[i].type == CAPI_NUMBER) {
            copy[out_i++] = hist->items[i].as.number;
        }
    }

    entry->value.as.array.values = copy;
    entry->value.as.array.len = numeric_count;
}
